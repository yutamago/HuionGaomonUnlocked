...
namespace HuionTablet
{
  public class FormHotKey : Form, IDestroy
  {
    ...
    private void imgTablet_Paint(object sender, PaintEventArgs e)
    {
      ...
      {
        e.Graphics.FillPath((Brush) solidBrush2, HuionRender.CreateRoundedRectanglePath(this.mCheckedKeyLayout.Rect, 2));
        e.Graphics.SmoothingMode = SmoothingMode.AntiAlias;
        e.Graphics.DrawLine(pen2, Utils.getViewCenter((Control) this.clickBtn, 16), Utils.getRectCenter(this.mCheckedKeyLayout.Rect));
      }
      pen2.Dispose();
    }
    ...
    private void FormHotKey_Load(object sender, EventArgs e)
    {
      ...
      this.imgTablet.Location = new Point((this.Width - this.imgTablet.Width) / 2, (this.Height - this.imgTablet.Height) / 2 - 20);
      ...
      FormHotKey.curEkeyIndex = 0;
      this.comboBoxMekey.Visible = false;
      this.keyLayouts = new HuionKeyLayout[(int) HNStruct.globalInfo.layoutTablet.hbtnNum + (int) HNStruct.globalInfo.layoutTablet.sbtnNum + (int) HNStruct.globalInfo.layoutTablet.mekeyNum];
      for (int index = 0; (long) index < (long) HNStruct.globalInfo.layoutTablet.hbtnNum; ++index)
      {
        this.keyLayouts[index] = Utils.swapLayout(this.imgTablet.Size, HNStruct.globalInfo.layoutTablet.size, HNStruct.globalInfo.hbtnLayouts[index]);
      ...
      {
        HuionKeyLayout huionKeyLayout = Utils.swapLayout(this.imgTablet.Size, HNStruct.globalInfo.layoutTablet.size, HNStruct.globalInfo.sbtnLayouts[index]);
      ...
      {
        HuionKeyLayout huionKeyLayout = Utils.swapLayout(this.imgTablet.Size, HNStruct.globalInfo.layoutTablet.size, HNStruct.globalInfo.mekeyLayouts[index]);
        huionKeyLayout.KeyType = HuionKeyType.MULTIKEY;
        huionKeyLayout.MutliKeys = HNStruct.globalInfo.mbtns;
        huionKeyLayout.KeyIndex = index;
        this.keyLayouts[(long) (HNStruct.globalInfo.layoutTablet.hbtnNum + HNStruct.globalInfo.layoutTablet.sbtnNum) + (long) index] = huionKeyLayout;
      }
    ...
    private void clickBtn_Click(object sender, EventArgs e)
    {
      ...
      {
        FormEKey_New formEkeyNew = new FormEKey_New((object) this.mCheckedKeyLayout, this.mCheckedKeyLayout.Key);
    ...
    {
      return this.mExcludeIndex1 == -1 || this.comboBoxMekey.SelectedIndex < this.mExcludeIndex1 ? this.comboBoxMekey.SelectedIndex : this.comboBoxMekey.SelectedIndex + 2;
    }
