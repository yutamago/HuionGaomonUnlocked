...
namespace HuionTablet
{
  public class FormCalibrate : Form
  {
    ...
    private void FormCalibrate_Load(object sender, EventArgs e)
    {
      ...
      this.Size = new Size(this.mScreenRect.Right - this.mScreenRect.Left, this.mScreenRect.Bottom - this.mScreenRect.Top);
      ...
      this.labelHint.Location = new Point((this.Width - this.labelHint.Width) / 2, (this.Height / 2 - this.labelHint.Height) / 2);
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
      ...
      for (int index = 0; index < this.mDisplayPoints.Length; ++index)
      {
        ...
        if (index == this.mCurrentPoint)
          ...
          break;
        Point point = new Point(mDisplayPoint.X + this.mPointSize.Width / 2, mDisplayPoint.Y + this.mPointSize.Height / 2);
    ...
    protected override void WndProc(ref Message m)
    {
      ...
      if (m.Msg != 74)
        return;
      this.mUserPoints[this.mCurrentPoint] = (HNStruct.HNPoint) Marshal.PtrToStructure(((HNStruct.COPYDATASTRUCT) Marshal.PtrToStructure(m.LParam, typeof (HNStruct.COPYDATASTRUCT))).lpData, typeof (HNStruct.HNPoint));
      ++this.mCurrentPoint;
    ...
    private void saveCalibrate(IntPtr ptr)
    {
      try
      {
        HuionDriverDLL.hnc_calibrate_monitor(ref HNStruct.globalInfo.tabletInfo, ref HNStruct.globalInfo.userConfig, ptr);
        int num = (int) HuionDriverDLL.hnd_save_config(ref HNStruct.globalInfo.userConfig, IntPtr.Zero);
      }
      catch (Exception ex)
      {
