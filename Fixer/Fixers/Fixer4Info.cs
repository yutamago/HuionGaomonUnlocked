...
namespace HuionTablet
{
  public class Fixer4Info
  {
    ...
    {
      if ((HNStruct.OemType != OEMType.GAOMON ? MessageBox.Show(ResourceCulture.GetString("FormInfo_RemindImportMessageText"), ResourceCulture.GetString("FormInfo_remindText"), MessageBoxButtons.OKCancel, MessageBoxIcon.Question) : buttonForm.importConfig()) == DialogResult.OK)
      {
    ...
    {
      if ((HNStruct.OemType != OEMType.GAOMON ? MessageBox.Show(ResourceCulture.GetString("FormInfo_RemindMessageText"), ResourceCulture.GetString("FormInfo_remindText"), MessageBoxButtons.OKCancel, MessageBoxIcon.Question) : buttonForm.defaultConfig()) == DialogResult.OK)
      {
        try
        {
          IntPtr coTaskMemAuto = Marshal.StringToCoTaskMemAuto(Application.StartupPath + "\\res\\config_default.xml");
          Marshal.AllocHGlobal(Marshal.SizeOf(typeof (HNStruct.HNConfigXML)));
          ...
          TabletConfigUtils.config = (HNStruct.HNConfigXML) Marshal.PtrToStructure(num2, typeof (HNStruct.HNConfigXML));
          new TabletConfigUtils().SetConfig(TabletConfigUtils.config);
          Marshal.FreeHGlobal(coTaskMemAuto);
          Marshal.FreeHGlobal(num1);
    ...
    {
      if (HNStruct.globalInfo.tabletInfo.devType == 34U || HNStruct.globalInfo.tabletInfo.devType == 37U || (HNStruct.globalInfo.tabletInfo.devType == 40U || HNStruct.globalInfo.tabletInfo.devType == 41U))
