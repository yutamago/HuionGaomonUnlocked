...
namespace HtmlAgilityPack
{
    public class HtmlDocument : IXPathNavigable
    {
        ...
        {
            get { return (IEnumerable<HtmlParseError>) this._parseerrors; }
        }
        ...
        public static string GetXmlName(string name)
        {
            ...
            for (int index = 0; index < name.Length; ++index)
            {
                if (name[index] >= 'a' && name[index] <= 'z' || name[index] >= '0' && name[index] <= '9' ||
                    (name[index] == '_' || name[index] == '-' || name[index] == '.'))
                {
        ...
        public static string HtmlEncode(string html)
        {
            ...
            return new Regex("&(?!(amp;)|(lt;)|(gt;)|(quot;))", RegexOptions.IgnoreCase).Replace(html, "&amp;")
        ...
        public Encoding DetectEncoding(TextReader reader)
        {
            ...
            this.Openednodes = !this.OptionCheckSyntax
                ? (Dictionary<int, HtmlNode>) null
                : new Dictionary<int, HtmlNode>();
            this.Nodesid = !this.OptionUseIdAttribute
                ? (Dictionary<string, HtmlNode>) null
        ...
        public void Load(TextReader reader)
        {
            ...
            foreach (HtmlNode htmlNode in this.Openednodes.Values)
            {
                if (htmlNode._starttag)
                {
                    ...
                    this.AddError(HtmlParseErrorCode.TagNotClosed, htmlNode._line, htmlNode._lineposition,
                        htmlNode._streamposition, sourceText, "End tag </" + htmlNode.Name + "> was not found");
                }
            }
        ...
        private void AddError(HtmlParseErrorCode code, int line, int linePosition, int streamPosition,
        ...
        private void CloseCurrentNode()
        {
            if (this._currentnode.Closed)
                return;
            bool flag = false;
            HtmlNode dictionaryValueOrNull =
                Utilities.GetDictionaryValueOrNull<string, HtmlNode>(this.Lastnodes, this._currentnode.Name);
            if (dictionaryValueOrNull == null)
            {
                if (HtmlNode.IsClosedElement(this._currentnode.Name))
                {
                    ...
                    if (this._lastparentnode != null)
                    {
                        ...
                        for (HtmlNode htmlNode2 = this._lastparentnode.LastChild;
                            htmlNode2 != null;
                ...
                {
                    this.AddError(HtmlParseErrorCode.EndTagNotRequired, this._currentnode._line,
                        this._currentnode._lineposition, this._currentnode._streamposition, this._currentnode.OuterHtml,
                        "End tag </" + this._currentnode.Name + "> is not required");
                }
                else
                {
                    this.AddError(HtmlParseErrorCode.TagNotOpened, this._currentnode._line,
                        this._currentnode._lineposition, this._currentnode._streamposition, this._currentnode.OuterHtml,
            ...
            else
            {
                if (this.OptionFixNestedTags &&
                    this.FindResetterNodes(dictionaryValueOrNull, this.GetResetters(this._currentnode.Name)))
                {
                    this.AddError(HtmlParseErrorCode.EndTagInvalidHere, this._currentnode._line,
                        this._currentnode._lineposition, this._currentnode._streamposition, this._currentnode.OuterHtml,
            ...
            if (flag || this._lastparentnode == null ||
                HtmlNode.IsClosedElement(this._currentnode.Name) && !this._currentnode._starttag)
                return;
        ...
        private void FixNestedTag(string name, string[] resetters)
        {
            ...
            if (dictionaryValueOrNull == null || this.Lastnodes[name].Closed ||
                this.FindResetterNodes(dictionaryValueOrNull, resetters))
                return;
            ...
            while (this._index < this.Text.Length)
            {
                ...
                switch (this._state)
                {
                    ...
                    case ParseState.Comment:
                        if (this._c == 62 && (!this._fullcomment ||
                                              this.Text[this._index - 2] == '-' && this.Text[this._index - 3] == '-'))
                        {
                            if (!this.PushNodeEnd(this._index, false))
                            {
                                this._index = this.Text.Length;
                    ...
                    case ParseState.PcData:
                        if (this._currentnode._namelength + 3 <= this.Text.Length - (this._index - 1) && string.Compare(
                                this.Text.Substring(this._index - 1, this._currentnode._namelength + 2),
                                "</" + this._currentnode.Name, StringComparison.OrdinalIgnoreCase) == 0)
                        {
                            ...
                            {
                                HtmlNode node = this.CreateNode(HtmlNodeType.Text,
        ...
        private bool PushNodeEnd(int index, bool close)
        {
            ...
            else if (this._currentnode._starttag && this._lastparentnode != this._currentnode)
            {
                ...
                this._currentnode._prevwithsamename =
                    Utilities.GetDictionaryValueOrNull<string, HtmlNode>(this.Lastnodes, this._currentnode.Name);
        ...
        private void ReadDocumentEncoding(HtmlNode node)
        {
            ...
            if (attribute1 == null ||
                string.Compare(attribute1.Value, "content-type", StringComparison.OrdinalIgnoreCase) != 0)
                return;
            ...
            if (this._streamencoding == null || this._declaredencoding == null ||
                this._declaredencoding.WindowsCodePage == this._streamencoding.WindowsCodePage)
                return;
            this.AddError(HtmlParseErrorCode.CharsetMismatch, this._line, this._lineposition, this._index,
                node.OuterHtml,
        ...
        public void Load(string path, Encoding encoding, bool detectEncodingFromByteOrderMarks, int buffersize)
        {
            ...
            using (StreamReader streamReader =
                new StreamReader(path, encoding, detectEncodingFromByteOrderMarks, buffersize))
                this.Load((TextReader) streamReader);
        }
