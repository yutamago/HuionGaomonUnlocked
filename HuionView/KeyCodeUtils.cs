...
namespace Huion
{
  public class KeyCodeUtils
  {
    ...
    {
      return KeyCodeUtils.isNumber(k) || KeyCodeUtils.isLetter(k) || (KeyCodeUtils.isFunction(k) || KeyCodeUtils.isOperator(k)) || (KeyCodeUtils.isDirection(k) || KeyCodeUtils.isOperation(k) || (KeyCodeUtils.isPunctuation(k) || KeyCodeUtils.isControls(k)));
    }
    ...
    {
      return k >= Keys.Snapshot && k <= Keys.Delete || (k == Keys.Space || k == Keys.Return) || (k == Keys.Escape || k >= Keys.Back && k <= Keys.Tab);
    }
    ...
    {
      return k >= Keys.ShiftKey && k <= Keys.Menu || k >= Keys.LShiftKey && k <= Keys.RMenu || k >= Keys.LWin && k <= Keys.RWin;
    }

    [DllImport("user32.dll")]
    public static extern int ToUnicode(uint virtualKeyCode, uint scanCode, byte[] keyboardState, [MarshalAs(UnmanagedType.LPWStr), Out] StringBuilder receivingBuffer, int bufferSize, uint flags);

    public static char convert2Char(Keys k)
    {
      StringBuilder receivingBuffer = new StringBuilder(2);
      byte[] keyboardState = new byte[2];
      KeyCodeUtils.ToUnicode((uint) k, 0U, keyboardState, receivingBuffer, 2, 0U);
      try
      {
