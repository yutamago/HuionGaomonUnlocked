...
namespace HuionTablet
{
  public class FormTabletPen : Form, IDestroy
  {
    ...
    private void pictureBoxPressCurve_Paint(object sender, PaintEventArgs e)
    {
      ...
      {
        graphics.DrawLine(pen2, (float) num1, (float) num2, (float) index, (float) ((long) this.pictureBoxPressCurve.Height - (long) HuionDriverDLL.hnc_calibrate_press_tablet(num3, ref HNStruct.globalInfo.tabletInfo, (uint) index, width)));
        num1 = index;
        num2 = (int) ((long) this.pictureBoxPressCurve.Height - (long) HuionDriverDLL.hnc_calibrate_press_tablet(num3, ref HNStruct.globalInfo.tabletInfo, (uint) index, width));
      }
    ...
    {
      this.Cursor = new Cursor(Huion.ViewUtils.CreateCursor());
    }
