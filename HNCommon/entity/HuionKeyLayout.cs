...
namespace HuionTablet.Entity
{
  public class HuionKeyLayout
  {
    ...
    {
      return x >= this.Rect.X && x <= this.Rect.X + this.Rect.Width && (y >= this.Rect.Y && y <= this.Rect.Y + this.Rect.Height) && (!(this.InnerRect != Rectangle.Empty) || x < this.InnerRect.X || (x > this.InnerRect.X + this.InnerRect.Width || y < this.InnerRect.Y) || y > this.InnerRect.Y + this.InnerRect.Height);
    }
