...
namespace HuionTablet.utils
{
  public class ScreenHelper
  {
    ...
    public static HNStruct.MONITORINFOEX[] MonitorInfos
    {
      get
      {
        return ScreenHelper.mMonitorInfos;
      }
    }
    ...
    public static int ScreenNum
    {
      get
      {
        return ScreenHelper.screenNum;
      }
    }

    public static void getMonitorInfos()
    {
      ScreenHelper.screenNum = 0;
      ScreenHelper.listener = new MonitorInfosCallback(ScreenHelper.onMonitorInfoCallback);
      ScreenHelper.mMonitorInfos = new HNStruct.MONITORINFOEX[10];
      HuionDriverDLL.EnumDisplayMonitors(IntPtr.Zero, IntPtr.Zero, ScreenHelper.listener, IntPtr.Zero);
      ...
      {
        Marshal.StructureToPtr((object) ScreenHelper.mMonitorInfos[index], new IntPtr(num3), false);
        num3 += (long) num1;
      }
      HuionDriverDLL.hnd_refresh_monitors(num2, (uint) ScreenHelper.screenNum);
      Marshal.FreeHGlobal(num2);
    }

    private static bool onMonitorInfoCallback(IntPtr hMonitor, IntPtr hdcMonitor, ref HNStruct.RECT lprcMonitor, IntPtr dwData)
    {
      ...
      ScreenHelper.mMonitorInfos[ScreenHelper.screenNum] = lpmi;
      ++ScreenHelper.screenNum;
      return true;
    }

    public static Bitmap getScreenImage(int index, HNStruct.MONITORINFOEX[] monitorinfo)
    {
      HNStruct.RECT screenRect = ScreenHelper.getScreenRect(index, monitorinfo);
    ...
    public static HNStruct.RECT getScreenRect(int index, HNStruct.MONITORINFOEX[] monitorinfo)
    {
      ...
      int num3 = 0;
      int num4 = 0;
      if (index == ScreenHelper.screenNum)
      {
        for (int index1 = 0; index1 < ScreenHelper.screenNum; ++index1)
        {
          if (num1 > ScreenHelper.MonitorInfos[index1].Monitor.Left)
            num1 = ScreenHelper.MonitorInfos[index1].Monitor.Left;
          if (num3 > ScreenHelper.MonitorInfos[index1].Monitor.Top)
            num3 = ScreenHelper.MonitorInfos[index1].Monitor.Top;
          if (num2 < ScreenHelper.MonitorInfos[index1].Monitor.Right)
            num2 = ScreenHelper.MonitorInfos[index1].Monitor.Right;
          if (num4 < ScreenHelper.MonitorInfos[index1].Monitor.Bottom)
            num4 = ScreenHelper.MonitorInfos[index1].Monitor.Bottom;
        }
      }
      else
      {
