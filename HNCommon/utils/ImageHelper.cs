...
namespace HuionTablet
{
  public class ImageHelper
  {
    ...
    {
      return (Image) ImageHelper.getImage(ImageHelper.GetIconPath(deviceTypeName, extensionStr));
    }

    public static Image GetScaleIco(string deviceTypeName, string extensionStr)
    {
      return (Image) HuionRender.blowupImage(ImageHelper.getImage(ImageHelper.GetIconPath(deviceTypeName, extensionStr)), DpiHelper.getInstance().XDpi);
    }

    public static Image GetScaleIco(string deviceTypeName, string extensionStr, Control v)
    {
      return ImageHelper.getScaleImage(ImageHelper.GetIconPath(deviceTypeName, extensionStr), v);
    }

    public static Image GetResizeIco(string deviceTypeName, string extensionStr, Control v)
    {
      return (Image) HuionRender.resizeImage(new Bitmap(ImageHelper.GetIco(deviceTypeName, extensionStr)), v.Width, v.Height);
    }
    ...
    public static Image getDllScaleImage(string imageName, Control v)
    {
      ...
      Bitmap bitmap = HuionRender.blowupImage(HuionRender.compressImageWithRate(bmp, v.Width, v.Height), DpiHelper.getInstance().XDpi);
      original.Dispose();
      if (bitmap == bmp)
        return (Image) bitmap;
      bmp.Dispose();
