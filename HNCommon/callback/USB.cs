...
namespace HuionTablet
{
    public class USB
    {
        ...
        {
            if (!HNDevices.ContainsKey(key))
                return;
            HNStruct.HNDevice hnDevice = HNDevices[key];
            hnDevice.Status = status;
            HNDevices.Remove(key);
            HNDevices.Add(key, hnDevice);
        }
        ...
        {
            ManagementObjectCollection objectCollection1 =
            ...
            foreach (ManagementBaseObject managementBaseObject in objectCollection1)
            {
                ...
                if (match.Success)
                {
                    ...
                    {
                        ManagementObjectCollection objectCollection2 =
                            new ManagementObjectSearcher("SELECT * FROM Win32_PnPEntity WHERE DeviceID=" + input).Get();
                        if (objectCollection2 != null)
                        {
                            foreach (ManagementObject managementObject in objectCollection2)
                            {
                                ...
                                if (!HNDevices.ContainsKey(input.Replace("\"", string.Empty)) &&
                                    HNStruct.HNDevice.isTabletDevice(hnDevice.Description))
                                    HNDevices.Add(input.Replace("\"", string.Empty), hnDevice);
                            }
                        }
                    }
                }
        ...
        public static bool hasConnectedDevice()
        {
            ...
            if (objectCollection != null)
            {
                ...
                {
                    Match match = Regex.Match((managementBaseObject["Dependent"] as string).Split('=')[1],
                        "VID_[0-9|A-F]{4}&PID_[0-9|A-F]{4}");
                    if (match.Success && HNStruct.HNDevice.isHNDevice(Convert.ToUInt16(match.Value.Substring(4, 4), 16),
                            Convert.ToUInt16(match.Value.Substring(13, 4), 16)))
                        return true;
                }
        ...
        public static bool isHNTabletDevice(EventArrivedEventArgs e)
        {
            ...
            if (managementBaseObject != null && managementBaseObject.ClassPath.ClassName == "Win32_USBControllerDevice")
            {
                ...
                Match match = Regex.Match(input, "VID_[0-9|A-F]{4}&PID_[0-9|A-F]{4}");
                if (match.Success &&
                    HNStruct.HNDevice.isHNDevice(Convert.ToUInt16(match.Value.Substring(4, 4), 16),
                        Convert.ToUInt16(match.Value.Substring(13, 4), 16)) &&
                    input.ToLower().IndexOf("vid_256c&pid_006e&mi_00") != -1)
                    return true;
            }
