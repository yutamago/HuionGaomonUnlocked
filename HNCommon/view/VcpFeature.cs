...
namespace HuionTablet
{
  public class VcpFeature
  {
    ...
    [DllImport("dxva2.dll")]
    [return: MarshalAs(UnmanagedType.Bool)]
    public static extern bool GetPhysicalMonitorsFromHMONITOR(IntPtr hMonitor, uint dwPhysicalMonitorArraySize, [Out] HNStruct.PHYSICAL_MONITOR[] pPhysicalMonitorArray);
    ...
    [DllImport("user32")]
    private static extern bool EnumDisplayMonitors(IntPtr hdc, IntPtr lpRect, VcpFeature.MonitorEnumProc callback, int dwData);

    [DllImport("dxva2.dll")]
    [return: MarshalAs(UnmanagedType.Bool)]
    private static extern bool GetVCPFeatureAndVCPFeatureReply(IntPtr hMonitor, byte bVCPCode, ref uint pvct, ref uint pdwCurrentValue, ref uint pdwMaximumValue);
    ...
    public static List<Monitors> GetAllMonitors()
    {
      ...
      VcpFeature.MonitorEnumProc callback = (VcpFeature.MonitorEnumProc) ((IntPtr hDesktop, IntPtr hdc, ref HNStruct.VCPRect r, int d) =>
      {
        uint pdwNumberOfPhysicalMonitors = 0;
        if (!VcpFeature.GetNumberOfPhysicalMonitorsFromHMONITOR(hDesktop, ref pdwNumberOfPhysicalMonitors))
          throw new Exception("无法获取显示器个数");
        HNStruct.PHYSICAL_MONITOR[] pPhysicalMonitorArray = new HNStruct.PHYSICAL_MONITOR[(int) pdwNumberOfPhysicalMonitors];
        if (!VcpFeature.GetPhysicalMonitorsFromHMONITOR(hDesktop, pdwNumberOfPhysicalMonitors, pPhysicalMonitorArray))
          throw new Exception("无法获取显示器句柄");
        physicalMonitors.Add(VcpFeature.GetMonitorCapabilities(new Monitors()
        {
          PhysicalMonitor = pPhysicalMonitorArray[0]
        }));
        return true;
      });
      VcpFeature.EnumDisplayMonitors(IntPtr.Zero, IntPtr.Zero, callback, 0);
    ...
    public static Monitors GetMonitorCapabilities(Monitors m)
    {
      ...
      bool flag = VcpFeature.GetCapabilitiesStringLength(m.PhysicalMonitor.hphysicalMonitor, ref pdwCapabilitiesStringLengthInCharacters);
      for (int index = 0; index < 4 && !flag; ++index)
      {
        Thread.Sleep(200);
        flag = VcpFeature.GetCapabilitiesStringLength(m.PhysicalMonitor.hphysicalMonitor, ref pdwCapabilitiesStringLengthInCharacters);
      }
      ...
      flag = VcpFeature.CapabilitiesRequestAndCapabilitiesReply(m.PhysicalMonitor.hphysicalMonitor, pszASCIICapabilitiesString, pdwCapabilitiesStringLengthInCharacters);
      for (int index = 0; index < 4 && !flag; ++index)
      {
        Thread.Sleep(200);
        flag = VcpFeature.CapabilitiesRequestAndCapabilitiesReply(m.PhysicalMonitor.hphysicalMonitor, pszASCIICapabilitiesString, pdwCapabilitiesStringLengthInCharacters);
      }
      return VcpFeature.ParseVcp(new string(pszASCIICapabilitiesString), m);
    }

    public static Monitors ParseVcp(string capabilityString, Monitors m)
    {
      capabilityString = VcpFeature.PrepareCapabilityString(capabilityString);
              ...
              else
              {
                ...
                if (hex != ")" && hex != "(")
                {
                  if (hex != "")
                  {
                    try
                    {
                      nullable = new uint?((uint) VcpFeature.HexToDecConverter(hex));
                      switch (vcpState)
                      {
    ...
    public static uint[] GetVCPFeature(Monitors m, HNStruct.VCPFeature vcpfeature)
    {
      ...
      bool andVcpFeatureReply = VcpFeature.GetVCPFeatureAndVCPFeatureReply(m.PhysicalMonitor.hphysicalMonitor, (byte) vcpfeature, ref pvct, ref pdwCurrentValue, ref pdwMaximumValue);
      for (int index = 0; index < 4 && !andVcpFeatureReply; ++index)
      {
        Thread.Sleep(200);
        andVcpFeatureReply = VcpFeature.GetVCPFeatureAndVCPFeatureReply(m.PhysicalMonitor.hphysicalMonitor, (byte) vcpfeature, ref pvct, ref pdwCurrentValue, ref pdwMaximumValue);
      }
    ...
    {
      bool flag = VcpFeature.SetVCPFeature(m.PhysicalMonitor.hphysicalMonitor, (byte) vcpfeature, newVal);
      for (int index = 0; index < 4 && !flag; ++index)
      {
        Thread.Sleep(200);
        flag = VcpFeature.SetVCPFeature(m.PhysicalMonitor.hphysicalMonitor, (byte) vcpfeature, newVal);
      }
      return flag;
    }
